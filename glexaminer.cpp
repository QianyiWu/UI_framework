...
GLExaminer::GLExaminer()
    :last_point_ok_(false), left_button_down_(false), right_button_down_(false), is_select(false), show_color_map(false)
{}
...
//  glClearColor(0.6f, 0.6f, 0.5f, 0.0f);
  ...
  //GLfloat mat_s[] = {0.1, 0.1, 0.1, 0.05};
  GLfloat shine[] = {128.0};
...
void GLExaminer::mousePressEvent(QMouseEvent* event)
{
    if(event->button() == Qt::LeftButton || event->button() == Qt::RightButton)
    {
        ...
        if (is_select)
        {
            ...
            gluUnProject(winX, winY, winZ, modelview_matrix_, projection_matrix_, viewport, &posX, &posY, &posZ);
            pX.push(posX); pY.push(posY); pZ.push(posZ);
            //再次绘制
            //glutPostRedisplay();
        }
    }
...
{
    if(event->buttons() == Qt::LeftButton && !is_select){
        rotation(event->x(), event->y());
    }
    else if (event->buttons() == Qt::LeftButton && is_select){
        GLfloat  winX, winY, winZ;
        GLdouble  posX, posY, posZ;
        winX = (float)event->x();
        winY = viewport[3] - (float)event->y();
        ...
        gluUnProject(winX, winY, winZ, modelview_matrix_, projection_matrix_, viewport, &posX, &posY, &posZ);
        pX.push(posX); pY.push(posY); pZ.push(posZ);
        //std::cout << "point:" << posX << " " << posY << " " << posZ << std::endl;
    }
